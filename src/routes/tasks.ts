...
const TASK_FIELDS = `id, name, normalized_name, score, days, archived_at, start_date, category, created_at, updated_at`;
...
router.post('/', requireAuth, async (req: Request, res: Response) => {
  const userId = (req as any).userId as string;
  const { name, score = 0, days = [], category = 'build', startDate = null } = req.body;
  if (!name) return res.status(400).json({ error: 'Missing name' });
  const normalized_name = String(name).trim().toLowerCase().replace(/\s+/g, ' ');
  try {
    const r = await pool.query(
      `INSERT INTO tasks (user_id, name, normalized_name, score, days, category, start_date)
       VALUES ($1,$2,$3,$4,$5,$6,$7)
       RETURNING ${TASK_FIELDS}`,
      [userId, name, normalized_name, score, JSON.stringify(days), category, startDate]
    );
